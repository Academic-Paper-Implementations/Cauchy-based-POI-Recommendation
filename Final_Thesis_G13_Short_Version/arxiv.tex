...
2. We incorporate the proposed prevalence measure into a clique-hash-table-based SCP mining pipeline. The pipeline uses the MCEHybrid algorithm, which is a hybrid Bron-Kerbosch-style maximal clique enumeration strategy between BK pivot and BK rcd, following prior graph mining studies \cite{bron1973algorithm,li2019fast}, to support efficient pattern discovery.
...
\subsection{Prevalence Metrics and Rarity Measures}
...
\subsection{Search Paradigms and Mining Algorithms}
...
For instance, as illustrated in Figure \ref{fig:spatial_network}, a synthetic spatial dataset consists of 17 spatial objects belonging to five feature categories ($\mathcal{T} = \{\mathrm{A}, \mathrm{B}, \mathrm{C}, \mathrm{D}, \mathrm{R}\}$): features $\mathrm{A}$ and $\mathrm{B}$ with 5 instances each, features $\mathrm{C}$ and $\mathrm{D}$ with 3 instances each, and a strictly rare feature $\mathrm{R}$ with only 1 instance. Here, the feature cardinalities are $\text{cnt}(\mathrm{A}) = 5$, $\text{cnt}(\mathrm{B}) = 5$, $\text{cnt}(\mathrm{C}) = 3$, $\text{cnt}(\mathrm{D}) = 3$, and $\text{cnt}(\mathrm{R}) = 1$.
...
% ---------------------------------------------------------------------
\noindent\textbf{Algorithmic Motivation and Pipeline Overview}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/mcr_pipeline.pdf}
...
\noindent\textbf{The MCR Pipeline Stages}
...
The grid itself is constructed in a single linear pass over $\mathcal{O}$ and requires only
$O(n)$ additional memory to store the cell-to-instance mapping. This spatial index is built once
and then reused for every neighbourhood query, so its construction cost is amortised across the
entire materialisation. In practice, the grid-based scheme therefore scales close to linearly with
the number of instances whenever the data are not pathologically clustered.
...
To demonstrate Definition 1, consider the synetic spatial dataset of
...
% ---------------------------------------------------------------------
\noindent\textbf{The MCR Algorithm}
...
% ---------------------------------------------------------------------
\noindent\textbf{An Illustrative Walkthrough}
...
\begin{figure}[htbp]
  ...
  \caption{The Co-located Spot Explorer.}
  \label{fig:software}
\end{figure}
...
thesis, not by a re-implementation or an approximation. Its purpose is to make
the co-location regularities the method discovers directly visible and
explorable: for any place on the map, the user sees which kinds of venues tend to
cluster around it, including the distinctive, comparatively rare categories that
the Cauchy weighting preserves.
...
and a popularity threshold, and runs the co-location search. The patterns are then
explored spatially: clicking any point on the map highlights the co-located venues,
each shown with its distance and user rating. In the example, the places co-located
with a selected seafood restaurant in Philadelphia are listed on the left by
distance, while the neighbourhood and search radius are drawn on the map. The
panel distinguishes venues that actually participate in a co-location with the
selected place from other venues that merely fall within the radius; because
co-location is defined between specific instances rather than entire categories,
a category may appear in both groups---some of its venues co-locate with the
selected place while others are only nearby.


\noindent\textbf{Application to POI recommendation.} In this application, the
mined co-location patterns act as signals for point-of-interest
recommendation. Rather than ranking venues in isolation, the system uses the
co-location relationships discovered by $WPI_C$ to answer a location-centered
