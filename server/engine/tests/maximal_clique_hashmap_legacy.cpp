/**
 * @file maximal_clique_hashmap_legacy.cpp
 * @brief Implementation: Standard Bron-Kerbosch with Pivot (LEGACY ORACLE)
 *
 * Rename-only copy of the pre-port src/maximal_clique_hashmap.cpp. Preserved as
 * the independent oracle for the raw maximal-clique set while the Fast-BK hybrid
 * port is validated by the diff_clique harness. Not part of the product binary.
 * Helpers live in an anonymous namespace (internal linkage), so co-linking with
 * the new src/maximal_clique_hashmap.cpp is safe.
 */
...
std::map<Colocation, std::unordered_map<FeatureType, std::set<const SpatialInstance*>>> MaximalCliqueHashmapLegacy::executeBK(
...
std::priority_queue<Colocation, std::vector<Colocation>, ColocationPriorityComp> MaximalCliqueHashmapLegacy::extractInitialCandidates(
