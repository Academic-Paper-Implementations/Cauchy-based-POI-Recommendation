...
// Instrumentation counter for the differential test harness only. Defined only
// when compiled with -DDIFF_CLIQUE_INSTRUMENT; the product binary is built
// without that macro, so these lines are preprocessed out and the shipped
// behaviour is unchanged.
#ifdef DIFF_CLIQUE_INSTRUMENT
long long g_diff_rcd_calls = 0;
#endif
