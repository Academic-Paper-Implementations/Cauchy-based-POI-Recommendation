...
std::vector<MinedPattern> Miner::minePCPs(
	...
	// Keyed by pattern so the accepted set matches the plain set the algorithm
	// used to return, while carrying the evidence behind each acceptance.
	std::map<Colocation, MinedPattern> prevalentPCs;
	...
	while (!candidateColocations.empty()) {
		...
		// A pattern deduced earlier can be reached again by the traversal. Keep
		// the measured value once it exists, whichever branch runs below.
		auto known = prevalentPCs.find(c);
		if (known != prevalentPCs.end()) {
			known->second.wpi = weightedPI;
			known->second.hasWpi = true;
		}

		if (weightedPI >= min_prev) {
			auto& record = prevalentPCs[c];
			record.features = c;
			record.wpi = weightedPI;
			record.hasWpi = true;
			record.deduced = false;  // justified by its own WPI

			auto prevalentSubsets = deducePrevalentSubsets(newCs, c, featureCounts);
			for (const auto& subset : prevalentSubsets) {
				auto inserted = prevalentPCs.emplace(subset, MinedPattern{ subset });
				if (inserted.second) {
					inserted.first->second.deduced = true;
				}
			}
	...
	results.reserve(prevalentPCs.size());
	for (const auto& entry : prevalentPCs) {
